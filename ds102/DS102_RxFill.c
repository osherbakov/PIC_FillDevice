...
static char GetQueryByte(void)
{
  ...
  while( is_not_timeout() )
  {
    ...
    if( PreviousState != NewState  )
    {
      if( NewState == LOW )
      {
		    Data = (Data >> 1 ) | (( pin_B() /*digitalRead(PIN_B) */ ) ? 0 : 0x80);
        bit_count++; 
...
static byte ReceiveDS102Cell(byte fill_type, byte *p_cell, byte count)
{
  ...
  while( is_not_timeout() &&  (byte_count < count) )
  {
    ...
    if( PreviousState != NewState  )
    {
      ...
      {
   		  set_timeout(tF);
  	    Data = (Data >> 1) | ( pin_D() /* digitalRead(PIN_D)*/  ? 0x00 : 0x80);  // Add Input data bit
        bit_count++; 
				if( bit_count >= 8)
				{
					*p_cell++ = Data;
...
char CheckFillType23()
{
  ...
  {
    case DF_INIT:
      if((pin_D() /* digitalRead(PIN_D) */  == HIGH) && (digitalRead(PIN_F) == HIGH))
      {
        t23_state = DF_HIGH;
      }
      break;
    ...
    case DF_HIGH:
      if(( pin_D() /* digitalRead(PIN_D) */  == LOW) && (digitalRead(PIN_F) == LOW))
      {
        t23_state = DF_LOW;
      }
      break;
    ...
    case DF_LOW:
    	...
    	while( is_not_timeout() )
    	{
          ...
          break;
    		...
    		if( pin_D() /* digitalRead(PIN_D) */  == HIGH)
    		{
    			type = 	GetQueryByte();
    			if( type > 0 )  
    			{
